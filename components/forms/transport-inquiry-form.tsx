...
export function TransportInquiryForm({ jobId }: { jobId: string }) {
  const form = useSubmissionForm({
    url: `/api/transport/jobs/${jobId}/inquiries`,
    validate: validateTransportInquiry,
    ...
    return (
      <ReceiptPanel
        ...
        links={[
          ...
          { href: `/transport/${jobId}`, label: '案件の詳細にもどる' },
        ]}
      />
    )
  }
