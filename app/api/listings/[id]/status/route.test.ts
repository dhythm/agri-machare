...
describe('PATCH /api/listings/[id]/status', () => {
  it('withdraws for the owner, hides it publicly, and republishes', async () => {
    const withdrawn = await patch('trc-001', { status: 'withdrawn' })
    expect(withdrawn.status).toBe(200)
    expect((await withdrawn.json()).withdrawnAt).toEqual(expect.any(String))
    signInAs(null)
    expect((await read('trc-001')).status).toBe(404)
    signInAs(demoSeller)
    expect((await read('trc-001')).status).toBe(200)
    expect((await patch('trc-001', { status: 'listed' })).status).toBe(200)
    expect((await patch('trc-001', { status: 'gone' })).status).toBe(400)
    signInAs(demoUser)
    expect((await patch('trc-001', { status: 'withdrawn' })).status).toBe(403)
    signInAs(demoAdmin)
    expect((await patch('missing', { status: 'withdrawn' })).status).toBe(404)
    signInAs(null)
    expect((await patch('trc-001', { status: 'withdrawn' })).status).toBe(401)
  })
})
