...
describe('listing detail business rules', () => {
  it('offers rental, rent-to-own, and purchase for eligible equipment', () => {
  ...
  it('only offers supported transactions', () => {
    expect(buildModes(getListing('drn-005')!).map((mode) => mode.id)).toEqual([
      'rent',
    ])
    expect(buildModes(getListing('trc-006')!).map((mode) => mode.id)).toEqual([
      'buy',
    ])
  })

  it('estimates transport by category, with a fallback for new categories', () => {
    const listing = getListing('trc-001')!
